...
Functions are implemented with the help of a \verb|VVPLCallable| class that allows us to handle each function call as its own instance; importantly, we pass on the provided arguments and the global variables in a new \verb|functionScope|. Within this scope, we can now execute the block body statements of the function. Importantly, we already check for the arity of the function parameters and provided argument in the \verb|ScopeAnalyzer| visitor, and for types of the function parameters and prodivded arguments in \verb|typeAnalyser|, so these checks are not performed.
