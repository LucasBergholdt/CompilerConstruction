...
In contrast, the testing of the interpretation process primarily consist of positive tests that confirm
...
This occurs because the error on line 10 causes the parser to enter panic mode.
During synchronization we deliberately do not synchronize on identifiers, since identifiers
do not guarantee the start of a new statement. As a result the entire assignment on line 12
...
that synchronization occurs only at grammar-guaranteed statement boundaries. 
Furthermore, it would still not make the test pass, as with this strategy we would also report the similar error on line 15,
which is not reported in the expected output.
We decided to stick with a simpler approach where the synchronise method returns when it is a grammar guarantee that a new statement has started.
...
We consider this acceptable, as the \texttt{then} statement could equivalently have appeared on line 21, making the reported error both valid and understandable.

The \texttt{sample-ast-expected.out} file fails on the final 3 lines, since we have a dedicated node for casts in the AST.
